...
int checkDirection(snake_t *snake, int32_t key) {
  ...
  // key может быть KEY_UP и т.п., а может быть буква
  int k = (int)key;
  if (k >= 0 && k <= 255)
    k = tolower(k);

  // определяем направление по кнопкам конкретной змеи
  if (k == snake->controls.up)
    new_dir = UP;
  else if (k == snake->controls.down)
    new_dir = DOWN;
  else if (k == snake->controls.left)
    new_dir = LEFT;
  else if (k == snake->controls.right)
    new_dir = RIGHT;
  else
    return 0; // не наша кнопка
