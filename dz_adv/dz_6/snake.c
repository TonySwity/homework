/*
 * 1. Доработать функционал игры "Змейка", реализовать в игре ситуацию, когда
 * змея врезается сама в себя. 2.Выход за границы экрана.
 ...
 *погибала при самопересечении. Для этого необходимо найти вызов функции
 *isCrush() и поменять вызов на нужное нам возвращаемое значение 0
 *3. Добавить цвет для двух змеек и еды.
 *4. Добавить стартовое меню (приветствие, выбор режима, выбор цвета змейки и
 *тд) Написать функцию void startMenu()
...
/*
 * 1. Develop Snake game functionality, implement situation when snake crashes
 * into itself.
 ...
 * self-intersection. For this, find isCrush() function call and change it to
 * return value 0
...
// No change dir 180
static int isOppositeDir(int cur, int next) {
  return (cur == LEFT && next == RIGHT) || (cur == RIGHT && next == LEFT) ||
         (cur == UP && next == DOWN) || (cur == DOWN && next == UP);
}
...
void changeAllDirections(snake_t snakes[], size_t num_snakes,
                         const int32_t key) {
  for (size_t i = 0; i < num_snakes; i++) {
    ...
    if (new_dir != 0 && !isOppositeDir(snakes[i].direction, new_dir)) {
...
void goTail(snake_t *head) {
  if (head->tsize == 0)
    return;
  if (!head->is_alive)
    return; // Don't move dead snake's tail
...
void drawAllSnakes(snake_t snakes[], size_t num_snakes) {
  for (size_t i = 0; i < num_snakes; i++) {
    if (!snakes[i].is_alive)
      continue; // Don't draw dead snakes
...
int isCrush(snake_t *snake) {
  ...
  // tail[0] у тебя хранит "предыдущую позицию головы", так что проверяем с 1
...
int checkWallCollision(snake_t *snake, int max_x, int max_y) {
  if (snake->x < 0 || snake->x >= max_x || snake->y < MIN_Y ||
      snake->y >= max_y) {
...
int checkSnakeEatSnake(snake_t snakes[], size_t num_snakes) {
  ...
  for (size_t i = 0; i < num_snakes; i++) {
    ...
    for (size_t j = 0; j < num_snakes; j++) {
      if (i == j || !snakes[j].is_alive)
        continue; // Skip self and dead
      ...
      for (size_t k = 0; k < snakes[j].tsize; k++) {
        if (snakes[i].x == snakes[j].tail[k].x &&
          ...
          snakes[j].tsize = (k == 0 ? 1 : k); // Shorten snake j's tail
          ...
          int max_x = 0, max_y = 0;
          getmaxyx(stdscr, max_y, max_x);
          mvprintw(4, 0, "Snake %zd bit tail of snake %zd! +20 points!", i + 1,
                   j + 1);
          break;
        }
      }
...
// int checkAllCollisions(snake_t snakes[], size_t num_snakes, int max_x,
//                        int max_y) {
//   int alive_count = 0;
...
int checkAllCollisions(snake_t snakes[], size_t num_snakes, int max_x,
  ...
  for (size_t i = 0; i < num_snakes; i++) {
    ...
    if (allow_self_death && checkWallCollision(&snakes[i], max_x, max_y)) {
      snakes[i].is_alive = 0;
      playSound(3);
      mvprintw(2, 0, "Snake %zd crashed into wall!              ", i + 1);
      continue;
    ...
    if (allow_self_death && isCrush(&snakes[i])) {
      snakes[i].is_alive = 0;
      playSound(3);
      mvprintw(2, 0, "Snake %zd crashed into itself!            ", i + 1);
    }
  ...
  // 2) механика "едят друг друга"
  if (can_eat_snakes)
    checkSnakeEatSnake(snakes, num_snakes);

  // 3) считаем живых ПОСЛЕ всего
  int alive_count = 0;
  for (size_t i = 0; i < num_snakes; i++)
    if (snakes[i].is_alive)
      alive_count++;
...
void showWelcomeScreen() {
  ...
  mvprintw(22, 8, "• Self-intersection: %s",
           allow_self_death ? "DEADLY" : "NOT deadly");
  mvprintw(23, 8, "• Boundaries: %s",
           allow_self_death ? "WALLS are deadly" : "Teleport (wrap)");
  mvprintw(24, 5, "Mode: %s (toggle in menu: H)",
           allow_self_death ? "HARD (self-crash kills)"
                            : "STANDARD (no self-crash)");
...
int startMenu() {
  ...
  char *color_names[] = {"Green",   "Red",  "Blue", "Yellow",
                         "Magenta", "Cyan", "White"};
  int num_colors = 7;
  ...
  while (1) {
    ...
    mvprintw(18, 5, "Mode: %s (H to toggle)",
             allow_self_death ? "HARD" : "STANDARD");
    mvprintw(19, 5, "Boundaries: %s", allow_self_death ? "WALLS" : "WRAP");
    ...
    mvprintw(27, 8,
             "Arrow keys ↑↓ - navigation, ENTER - select, TAB - switch snakes");
    ...
    case 'h':
    case 'H':
      allow_self_death = !allow_self_death;
      break;
    case 'm':
    case 'M':
...
static int cellBusy(int x, int y, snake_t snakes[], size_t n) {
  for (size_t i = 0; i < n; i++) {
    if (!snakes[i].is_alive)
      continue;
    if (snakes[i].x == x && snakes[i].y == y)
      return 1;
    for (size_t k = 0; k < snakes[i].tsize; k++)
...
void spawnFoodSafe(food_t *food, int max_x, int max_y, snake_t snakes[],
                   size_t n) {
  for (int tries = 0; tries < 2000; tries++) {
    int x = rand() % max_x;
    int y = (rand() % (max_y - MIN_Y)) + MIN_Y;
    if (!cellBusy(x, y, snakes, n)) {
      food->x = x;
      food->y = y;
      food->symbol = '#';
      return;
  ...
  // fallback если экран забит
  food->x = 0;
  food->y = MIN_Y;
  food->symbol = '#';
}

static void wrapSnake(snake_t *s, int max_x, int max_y) {
  if (s->x < 0)
    s->x = max_x - 1;
  else if (s->x >= max_x)
    s->x = 0;
...
void playGame() {
  ...
  nodelay(stdscr, TRUE);
  int frame_ms = 120;
  long long next_tick = now_ms() + frame_ms;
  ...
  spawnFoodSafe(&food, max_x, max_y, snakes, NUM_SNAKES);
  ...
  while (key_pressed != STOP_GAME && !game_over && game_running) {
    ...
    // 1) Считываем клавиши сколько успеем (не блокируясь)
    int ch;
    while ((ch = getch()) != ERR) {
      key_pressed = ch;
    ...
    if (now >= next_tick) {
      ...
      for (size_t i = 0; i < NUM_SNAKES; i++) {
        if (snakes[i].is_alive) {
          ...
          go(&snakes[i]);
          if (!allow_self_death) {
            wrapSnake(&snakes[i], max_x, max_y);
          }
        }
      }
